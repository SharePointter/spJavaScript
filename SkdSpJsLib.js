//====================================================================================================================UTILITIES
		SkdSpJs.Utils = {
					...
					*/
					...
					function _resizeDlg(){
							...
							if (dlg != null) {
									...
									$('.ms-dlgTitle').css({'padding':'10px 30px 5px 5px'});
							}else{
					...
					function resizeDlg(cntr){
							...
							if(dlg !=null && dlg.$e_0 != null){
									...
									if(dom){
											...
											cntr = cntr + 1;
											cntr < 25 ? setTimeout(function(){ resizeDlg(cntr);},200) : console.log('DLG Timed Out');
									}
							}else{
									cntr = cntr + 1;
									cntr < 20 ? setTimeout(function(){ resizeDlg(cntr);},200) : console.log('DLG Timed Out');
							}
					}
			...
			dragAndDropCheckInFxn: function (_ctx) {
					...
					var _newDndFxn = function () {
							var fileArr = _ctx.ListData.Row.map(function(e){
									...
									fileId = e.ID;
									fileType = e.fieldName
									var editFormUrl = currUrl + fileId + '&Source=' + window.location.href;
									if (fileType === 'dd option' || fileType === undefined) {
			...
			skdDownloadAllAttachments: function (n){
				...
				isDownloaded ? setTimeout( function(){ skdDownloadAllAttachments(n+1); }, 1000 ) : '';
			}
		};
//====================================================================================================================JSOM
		SkdSpJs.Jsom = {
				...
				deleteNavigationNode: function (){
						...
						_ctx.executeQueryAsync(
								function(){
										...
										_ctx.executeQueryAsync(
												function(sender, args){console.log(sender, args);},
												function(sender, args){console.log(sender, args);}
										);
										str = str+']';
										console.log(str);
		...
		SkdSpJs.Rest = {
				...
				getCurrentUserProfile: function(){
						$.ajax({
								...
								headers: { "Accept": "application/json; odata=verbose" },
								success: function(data) {
										pos = data.d.UserProfileProperties.results.map(function(e) { return e.Key; }).indexOf('EmployeeNumber');
				...
				createField: function (colName){
						var _data = { '__metadata': { 'type': 'SP.Field' }, 'FieldTypeKind': 9, 'Title': colName };
						return $.ajax({
				...
				addFieldToView: function (colName){
						return $.ajax({
								...
								type: "POST",
								data: JSON.stringify({ 'strField':colName }),
								headers:{
										"accept": "application/json;odata=verbose",
										"content-Type": "application/json;odata=verbose",
